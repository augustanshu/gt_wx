...
if ($_W['ispost']) {
	...
	$list = pdo_fetchall('SELECT *  FROM ' . tablename('ewei_dshop_exchange_goods') . " WHERE 1 {$condition} ORDER BY id LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	$total = pdo_column('SELECT count(*) FROM ' . tablename('ewei_dshop_exchange_goods') . " WHERE 1 {$condition}");
