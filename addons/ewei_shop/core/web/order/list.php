...
if ($operation == 'display') {
	...
	if ($status != '') {
		...
		if ($status == '-1') {
			$condition .= " AND o.status=-1 and r.status!=1";
		} else {
			if ($status == '4') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . " o " . " left join " . tablename('ewei_shop_order_refund') . " r on r.orderid=o.id" . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid " . " left join " . tablename('ewei_shop_member_address') . " a on o.addressid = a.id " . " WHERE {$condition}", $paras);
	$totalmoney = pdo_fetchcolumn('SELECT sum(o.price) FROM ' . tablename('ewei_shop_order') . " o " . " left join " . tablename('ewei_shop_order_refund') . " r on r.orderid=o.id" . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid  " . " left join " . tablename('ewei_shop_member_address') . " a on o.addressid = a.id " . " WHERE {$condition}", $paras);
...
function order_list_confirmsend($item)
{
	...
	if (!empty($item['refundid'])) {
		...
		if (!empty($refund)) {
			...
			pdo_update('ewei_shop_order', array('refundid' => 0), array('id' => $item['id']));
		}
	}
		...
		if ($refundstatus == 1) {
			...
			if (!empty($refundtype)) {
				if ($item['deductcredit2'] > 0) {
					m('member')->setCredit($item['openid'], 'credit2', $item['deductcredit2'], array('0', $shopset['name'] . "购物返还抵扣余额 积分: {$item['deductcredit2']} 订单号: {$item['ordersn']}"));
				}
			}
