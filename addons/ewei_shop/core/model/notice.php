...
class Ewei_DShop_Notice
{
			...
			if (!empty($order['refundid'])) {
				...
				if (empty($refund['status'])) {
					...
					if ($refund['status'] == 1) {
						...
						$msg = array('first' => array('value' => "您的订单已经完成退款！", "color" => "#4a5077"), 'orderProductPrice' => array('title' => '退款金额', 'value' => '¥' . $refund['price'] . '元', "color" => "#4a5077"), 'orderProductName' => array('title' => '商品详情', 'value' => $goods, "color" => "#4a5077"), 'orderName' => array('title' => '订单编号', 'value' => $order['ordersn'], "color" => "#4a5077"), 'remark' => array('value' => "\r\n 退款金额 ¥" . $refund['price'] . ", 已经退回您的{$refundtype}，请留意查收！\r\n 【" . $shop['name'] . "】期待您再次购物！", "color" => "#4a5077"));
	...
	public function sendMemberLogMessage($log_id = '')
	{
		...
		if ($log_info['type'] == 0) {
			...
			if ($log_info['type'] == 1 && $log_info['status'] == 0) {
