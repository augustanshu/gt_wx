...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		if ($operation == 'new') {
			$member = fans_search($openid, array('resideprovince', 'residecity'));
			show_json(1, array('address' => array('province' => $member['resideprovince'], 'residecity' => $member['residecity']), 'member' => $member, 'shareAddress' => $shareAddress));
		} else {
			if ($operation == 'get') {
				$id = intval($_GPC['id']);
				...
				$member = fans_search($openid, array('residecity'));
				...
				if ($operation == 'submit' && $_W['ispost']) {
					...
					if ($operation == 'remove' && $_W['ispost']) {
						...
						if ($operation == 'setdefault' && $_W['ispost']) {
							$id = intval($_GPC['id']);
