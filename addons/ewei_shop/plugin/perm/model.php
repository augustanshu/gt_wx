		...
		function check_plugin($pluginname = '')
		{
			...
			if (!empty($type)) {
				...
				if (!empty($ac_perm)) {
					...
					$user_perm = pdo_fetch('select  plugins from ' . tablename('ewei_shop_perm_plugin') . ' where uid=:uid limit 1', array(':uid' => $_W['uid']));
					if (!empty($user_perm)) {
