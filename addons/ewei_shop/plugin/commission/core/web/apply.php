...
if ($operation == 'display') {
	...
	$list = pdo_fetchall("select a.*, m.nickname,m.avatar,m.realname,m.mobile,l.levelname from " . tablename('ewei_shop_commission_apply') . " a " . " left join " . tablename('ewei_shop_member') . " m on m.id = a.mid" . " left join " . tablename('ewei_shop_commission_level') . " l on l.id = m.agentlevel" . " where 1 {$condition} ORDER BY {$orderby} desc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
	...
	if ($operation == 'detail') {
		...
		if (checksubmit('submit_check') && $apply['status'] == 1) {
			...
			$ogids = array();
			foreach ($list as $row) {
				$goods = pdo_fetchall("SELECT id from " . tablename('ewei_shop_order_goods') . " where uniacid = :uniacid and orderid=:orderid", array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
				foreach ($goods as $g) {
					$ogids[] = $g['id'];
				}
			}
			...
			foreach ($ogids as $ogid) {
				$g = pdo_fetch("SELECT total, commission1,commission2,commission3 from " . tablename('ewei_shop_order_goods') . "  " . "where id=:id and uniacid = :uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
				if (empty($g)) {
					continue;
				}
				if ($this->set['level'] >= 1) {
