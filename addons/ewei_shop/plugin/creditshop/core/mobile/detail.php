...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		if ($operation == 'pay' && $_W['ispost']) {
			...
			$log = array('uniacid' => $uniacid, 'openid' => $openid, 'logno' => m('common')->createNO('creditshop_log', 'logno', empty($goods['type']) ? 'EE' : 'EL'), 'goodsid' => $id, 'status' => 0, 'paystatus' => $goods['money'] > 0 ? 0 : -1, 'dispatchstatus' => $goods['dispatch'] > 0 ? 0 : -1, 'createtime' => time());
			if (empty($goods['type'])) {
